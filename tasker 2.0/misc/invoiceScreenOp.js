...
import { initialInvoiceNo } from './startData/dataConfig';
...
function InvoiceScreen() {
    ...
    function handleItemSelect(e, index) {
      ...
      console.log("selected:", selectedValue, "index:", index);
    ...
    const tableRows = rows.map((row, index) => (
      <tr key={index}>
        ...
        <td className="invoice-table-cell invoice-table-item-cell">
        <input
          type="text"
        ...
        <td className="invoice-table-cell amount-cell">
          <input
            ...
            className="invoice-input price-input"
            step="0.01"
            value={row.invoice_itemTotal.toFixed(2)}
            readOnly={true}
          />
        </td>
        <td className="invoice-table-cell">
